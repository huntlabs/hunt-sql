...
// import hunt.sql.dialect.hive.ast.HiveInsertStatement;
...
public class SQLStatementParser : SQLParser {
    ...
    protected bool            parseCompleteValues = true;
    ...
    protected this(Lexer lexer, string dbType){
        super(lexer, dbType);
    }
    ...
    public void parseStatementList(List!(SQLStatement) statementList, int max, SQLObject parent) {
        import std.stdio;
        // writeln("#####");
        if ("select @@session.tx_read_only" == (lexer.text)
        ...
        for (;;) {
            ...
            switch (lexer.token) {
                ...
                case Token.COMMENT: {
                    if(DBType.MYSQL.opEquals(this.dbType)){//mysql 关键字 comment 没有这个语法，oracle才有
                        return;
                    }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ROLLBACK)) {
                ...
                if (cast(SQLBlockStatement)(parent) !is null
                        && DBType.MYSQL.opEquals(dbType)) {
                    return;
                }
    ...
    public SQLGrantStatement parseGrant() {
        ...
        if (lexer.token == Token.ON) {
            ...
            if (stmt.getObjectType().name.length != 0 && lexer.token == Token.COLONCOLON) {
            ...
            if (stmt.getObjectType() == SQLObjectType.TABLE || stmt.getObjectType().name.length == 0) {
            ...
            for (;;) {
                if (lexer.token == Token.DROP) {
                    ...
                    if (lexer.token == Token.PRIMARY) {
                        ...
                        parseAssignItems(cast(List!SQLAssignItem)(addPartition.getPartitions()), cast(SQLObject)addPartition);
                    ...
                    if (lexer.identifierEquals("RENAME")) {
                        ...
                        if (lexer.token == Token.LITERAL_ALIAS) {
                            ...
                            comment = new SQLCharExpr(_alias);
                            lexer.nextToken();
                        } else {
                    ...
                    if (lexer.token == Token.ENABLE) {
                        ...
                        foreach(SQLAssignItem condition ; renamePartition.getPartition()) {
                    ...
                    stmt.addItem(item);
                } else if (DBType.ODPS.opEquals(dbType) && lexer.identifierEquals("MERGE")) {
                    lexer.nextToken();
    ...
    protected SQLAlterTableDropPartition parseAlterTableDropPartition(bool ifExists) {
        lexer.nextToken();
        ...
        if (lexer.token == Token.LPAREN) {
            ...
            parseAssignItems(cast(List!SQLAssignItem)(dropPartition.getPartitions()), cast(SQLObject)dropPartition);
    ...
    protected SQLDropTableStatement parseDropTable(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected SQLDropSequenceStatement parseDropSequence(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected SQLDropTriggerStatement parseDropTrigger(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected SQLDropViewStatement parseDropView(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected SQLDropDatabaseStatement parseDropDatabase(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected SQLDropFunctionStatement parseDropFunction(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected SQLDropTableSpaceStatement parseDropTablespace(bool acceptDrop) {
    ...
    protected SQLDropProcedureStatement parseDropProcedure(bool acceptDrop) {
        if (acceptDrop) {
            accept(Token.DROP);
        }
    ...
    protected void parseInsert0(SQLInsertInto insertStatement, bool acceptSubQuery) {
        if (lexer.token == Token.INTO) {
            lexer.nextToken();
    ...
    public bool parseStatementListDialect(List!(SQLStatement) statementList) {
        return false;
    }
    ...
    public SQLStatement parseCreateMaterializedView() {
        ...
        for (;;) {
            // if (cast(OracleExprParser)(exprParser) !is null) {
            ...
            if (lexer.identifierEquals("REFRESH")) {
                ...
                if (lexer.token == Token.LITERAL_INT) {
                    stmt.setParallelValue(cast(Integer)(lexer.integerValue()));
                    lexer.nextToken();
                }
    ...
    public SQLStatement parseCreateTrigger() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.DEFINER)) {
            lexer.nextToken();
            accept(Token.EQ);
            SQLName definer = (cast(MySqlExprParser) this.exprParser).userName();
    ...
    public SQLStatement parseCreateSequence(bool acceptCreate) {
        throw new ParserException("TODO " ~ lexer.token);
    }

    public SQLStatement parseCreateIndex(bool acceptCreate) {
        if (acceptCreate) {
            accept(Token.CREATE);
        }
    ...
    public SQLCreateViewStatement parseCreateView() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.DEFINER)) {
            lexer.nextToken();
            accept(Token.EQ);
            SQLName definer = cast(SQLName) (cast(MySqlExprParser) this.exprParser).userName();
    ...
    protected SQLAlterTableAddColumn parseAlterTableAddColumn() {
        bool odps = DBType.ODPS.opEquals(dbType);
    ...
    */
    public SQLStatement parseStatement(  bool tryBest) {
    ...
    public bool isParseCompleteValues() {
        return parseCompleteValues;
    }

    public void setParseCompleteValues(bool parseCompleteValues) {
    ...
    public SQLStatement parseWith() {
        SQLWithSubqueryClause _with = this.parseWithQuery();
    ...
    protected void parseValueClause(List!(ValuesClause) valueClauseList, int columnSize, SQLObject parent) {
         bool optimizedForParameterized = lexer.isEnabled(SQLParserFeature.OptimizedForForParameterizedSkipValue);
