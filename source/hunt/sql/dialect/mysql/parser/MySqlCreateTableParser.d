 ...
 */
module hunt.sql.dialect.mysql.parser.MySqlCreateTableParser;
...
import hunt.sql.ast.SQLPartitionByHash;
import hunt.sql.ast.SQLPartitionByList;
import hunt.sql.ast.SQLPartitionByRange;
import hunt.sql.ast.SQLSubPartitionBy;
import hunt.sql.ast.SQLSubPartitionByHash;
...
import hunt.sql.dialect.mysql.ast.statement.MySqlPartitionByKey;
import hunt.sql.dialect.mysql.ast.statement.MySqlSubPartitionByKey;
import hunt.sql.dialect.mysql.ast.statement.MySqlSubPartitionByList;
...
public class MySqlCreateTableParser : SQLCreateTableParser {
    ...
    override public MySqlCreateTableStatement parseCreateTable(bool acceptCreate) {
                ...
                for (;;) {
                    if (lexer.token() == Token.FULLTEXT) {
                        ...
                        if (lexer.token() == Token.KEY) {
                            MySqlKey fulltextKey = cast(MySqlKey) parseConstraint();
                        ...
                        if (lexer.token() == Token.INDEX) {
                            ...
                            if (lexer.token() == Token.IDENTIFIER) {
                                if (!equalsIgnoreCase("USING",lexer.stringVal())) {
                                    idx.setName(this.exprParser.name());
                                }
                            }
                    ...
                    if (lexer.token() != Token.COMMA) {
                        ...
                        lexer.nextToken();
                        if (lexer.isKeepComments() && lexer.hasComment() && column !is null) {
                            column.addAfterComment(lexer.readAndResetComments());
                        }
                    }
                }
        ...
        if (lexer.identifierEquals("SUBPARTITION")) {
            ...
            if (lexer.token() == Token.KEY) {
                ...
                if (lexer.token() == Token.LPAREN) {
                    ...
                    if (cast(SQLIdentifierExpr)(expr) !is null && (lexer.identifierEquals("bigint") || lexer.identifierEquals("long"))) {
                        string dataType = lexer.stringVal();
                        lexer.nextToken();

                        SQLColumnDefinition column = this.exprParser.createColumnDefinition();
                        column.setName(cast(SQLIdentifierExpr) expr);
            ...
            if (subPartitionByClause !is null) {
    ...
    private bool parseTableOptionCharsetOrCollate(MySqlCreateTableStatement stmt) {
    ...
    override protected SQLTableConstraint parseConstraint() {
        SQLName name = null;
        bool hasConstaint = false;
