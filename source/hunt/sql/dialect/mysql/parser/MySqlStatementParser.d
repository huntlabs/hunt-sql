 ...
 */
module hunt.sql.dialect.mysql.parser.MySqlStatementParser;

import hunt.sql.SQLUtils;
import hunt.sql.ast;
import hunt.sql.ast.SQLParameter;
// import hunt.sql.ast.SQLParameter.ParameterType;
...
// import hunt.sql.dialect.mysql.ast.statement.MySqlLockTableStatement.LockType;
...
public class MySqlStatementParser : SQLStatementParser {
    ...
    override public SQLStatement parseSelect() {
        ...
        return new SQLSelectStatement(select, DBType.MYSQL.name);
    }
    ...
    override protected MySqlUpdateStatement createUpdateStatement() {
        return new MySqlUpdateStatement();
    }

    override public MySqlDeleteStatement parseDeleteStatement() {
    ...
    override public SQLStatement parseCreate() {
...
//            Lexer.SavePoint savePoint = lexer.mark();
    ...
    override public SQLStatement parseCreateUser() {
        if (lexer.token() == Token.CREATE) {
            lexer.nextToken();
        }
        ...
        for (; ; ) {
            ...
            if (lexer.token() == Token.IDENTIFIED) {
                ...
                if (lexer.token() == Token.BY) {
                    ...
                    SQLCharExpr password = cast(SQLCharExpr) this.exprParser.expr();
    ...
    override public SQLStatement parseKill() {
        accept(Token.KILL);
    ...
    override public bool parseStatementListDialect(List!(SQLStatement) statementList) {
        ...
        if (lexer.token() == Token.LOCK) {
            ...
            bool isLockTables = equalsIgnoreCase(TABLES, val) && lexer.token() == Token.IDENTIFIER;
            bool isLockTable = equalsIgnoreCase("TABLE",val) && lexer.token() == Token.TABLE;
        ...
        if (lexer.identifierEquals("UNLOCK")) {
            lexer.nextToken();
            string val = lexer.stringVal();
            bool isUnLockTables = equalsIgnoreCase(TABLES, val) && lexer.token() == Token.IDENTIFIER;
            bool isUnLockTable = equalsIgnoreCase("TABLE",val) && lexer.token() == Token.TABLE;
        ...
        if (lexer.token() == Token.HINT) {
            ...
            bool tddlHints = false;
            bool accept = false;


            bool acceptHint = false;
            switch (lexer.token()) {
                case Token.SELECT:
                case Token.WITH:
                case Token.DELETE:
            ...
            if (hints.size() == 1
                ...
                string hintText = toUpper(hint.getText());
                if (startsWith(hintText,"+TDDL")
                        || startsWith(hintText,"+ TDDL")
                        || startsWith(hintText,"TDDL")
                        || startsWith(hintText,"!TDDL"))
                {
                    tddlHints = true;
                } else if(startsWith(hintText,"+")) {
                    accept = true;
                }
            }

            if (tddlHints) {
                SQLStatementImpl stmt = cast(SQLStatementImpl)this.parseStatement();
    ...
    override public SQLBlockStatement parseBlock() {
        ...
        if (lexer.token() != Token.END
                && statementList.size() > 0
                && (cast(SQLCommitStatement)statementList.get(statementList.size() - 1) !is null
                    || cast(SQLRollbackStatement)statementList.get(statementList.size() - 1) !is null)) {
    ...
    override public MySqlExplainStatement parseDescribe() {
    ...
    override public MySqlExplainStatement parseExplain() {
        // see https://dev.mysql.com/doc/refman/5.7/en/explain.html
    ...
    private MySqlExplainStatement parseExplain(MySqlExplainStatement explain) {
        ...
        if (lexer.token() == Token.IDENTIFIER) {
             ...
             if (equalsIgnoreCase(stringVal, EXTENDED)
                     || equalsIgnoreCase(stringVal, PARTITIONS)) {
                 explain.setType(stringVal);
                 lexer.nextToken();
             } else if (equalsIgnoreCase(stringVal, FORMAT)) {
                 explain.setType(stringVal);
                 lexer.nextToken();
                 accept(Token.EQ);
    ...
    override public SQLStatement parseShow() {
        ...
        if (lexer.identifierEquals(ENGINE)) {
            ...
            stmt.setName(this.exprParser.name());
            stmt.setOption(MySqlShowEngineStatement.Option(toUpper(lexer.stringVal())));
            lexer.nextToken();
            return stmt;
        }
    ...
    private SQLShowTablesStatement parseShowTabless() {
        ...
        if (lexer.token() == Token.FROM || lexer.token() == Token.IN) {
            lexer.nextToken();
            SQLName database = exprParser.name();
            if (lexer.token() == Token.SUB && cast(SQLIdentifierExpr)(database) !is null) {
                lexer.mark();
                lexer.nextToken();
                string strVal = lexer.stringVal();
                lexer.nextToken();
                if (cast(SQLIdentifierExpr)(database) !is null) {
                    SQLIdentifierExpr ident = cast(SQLIdentifierExpr) database;
    ...
    override public SQLStatement parseCommit() {
    ...
    protected MySqlLoadXmlStatement parseLoadXml() {
        ...
        SQLLiteralExpr fileName = cast(SQLLiteralExpr) exprParser.expr();
    ...
    override public SQLInsertStatement parseInsert() {
        ...
        int columnSize = 0;
        if (lexer.token() == Token.LPAREN) {
            bool useInsertColumnsCache = lexer.isEnabled(SQLParserFeature.UseInsertColumnsCache);
            ...
            if (cachedColumns !is null
                ...
                int p2 = pos + cast(int)(cachedColumns.columnsString.length);
                lexer.reset(p2);
                lexer.nextToken();
            } else {
                lexer.nextToken();
                if (lexer.token() == Token.SELECT) {
                    ...
                    if (lexer.token() != Token.RPAREN) {
                        ...
                        if (insertColumnsCache !is null && tableName !is null) {
        ...
        if (lexer.token() == Token.ON) {
            ...
            for (;;) {
                ...
                try {
                    ...
                    throw new ParserException("EOF, " ~ name.stringof ~ "=", e);
                }
    ...
    override public MySqlSelectParser createSQLSelectParser() {
        return new MySqlSelectParser(this.exprParser, selectListCache);
    }

    override public SQLStatement parseSet() {
        accept(Token.SET);
...
//            return stmt;
            ...
            if (global !is null && global) {
                SQLVariantRefExpr varRef = cast(SQLVariantRefExpr) stmt.getItems().get(0).getTarget();
    ...
    override public SQLStatement parseAlter() {
        accept(Token.ALTER);
    ...
    protected SQLStatement parseAlterView() {
        ...
        if (lexer.identifierEquals("DEFINER")) {
            lexer.nextToken();
            accept(Token.EQ);
            SQLName definer = cast(SQLName) this.exprParser.expr();
        ...
        if (lexer.token() == Token.LPAREN) {
            ...
            for (;;) {
                ...
                if (lexer.token() == Token.CONSTRAINT) {
                    SQLTableConstraint constraint = cast(SQLTableConstraint) this.exprParser.parseConstaint();
                    ...
                    if (lexer.token() == Token.COMMENT) {
                        ...
                        if (lexer.token() == Token.LITERAL_ALIAS) {
                            string alias_p = lexer.stringVal();
                            if (alias_p.length > 2 && charAt(alias_p, 0) == '"' && charAt(alias_p, alias_p.length - 1) == '"') {
                                alias_p = alias_p.substring(1, cast(int)(alias_p.length - 1));
                            }
                            comment = new SQLCharExpr(alias_p);
                            lexer.nextToken();
                        } else {
        ...
        if (lexer.token() == Token.COMMENT) {
            lexer.nextToken();
            SQLCharExpr comment = cast(SQLCharExpr) exprParser.primary();
    ...
    override protected SQLStatement parseAlterFunction() {
        if (lexer.token() == Token.ALTER) {
            lexer.nextToken();
        }
    ...
    protected SQLStatement parseCreateEvent() {
        ...
        if (lexer.token() == Token.ENABLE) {
            stmt.setEnable(new Boolean(true));
            lexer.nextToken();
        } else if (lexer.token() == Token.DISABLE) {
            lexer.nextToken();
            stmt.setEnable(new Boolean(false));
    ...
    protected SQLStatement parseAlterTable(bool ignore) {
        lexer.nextToken();
            ...
            if (lexer.token() == Token.DROP) {
                ...
                if (lexer.token() == Token.COLUMN) {
                    ...
                    if (lexer.token() == Token.PRIMARY) {
                        SQLPrimaryKey primaryKey = (cast(MySqlExprParser) this.exprParser).parsePrimaryKey();
                        SQLAlterTableAddConstraint item = new SQLAlterTableAddConstraint(primaryKey);
                        item.setParent(stmt);
                ...
                if (stmt.getItems().size() > 0) {
                    ...
                    item.setName(cast(SQLName) stmt.getTableSource().getExpr());
    ...
    override public void parseAlterDrop(SQLAlterTableStatement stmt) {
        lexer.nextToken();
        if (lexer.token() == Token.INDEX) {
            lexer.nextToken();
    ...
    override public SQLStatement parseRename() {
    ...
    override public SQLStatement parseCreateDatabase() {
        if (lexer.token() == Token.CREATE) {
            lexer.nextToken();
        }
        ...
        SQLCreateDatabaseStatement stmt = new SQLCreateDatabaseStatement(DBType.MYSQL.name);
    ...
    override protected void parseUpdateSet(SQLUpdateStatement update) {
        accept(Token.SET);
    ...
    override public MySqlExprParser getExprParser() {
        return cast(MySqlExprParser) exprParser;
    }


    override public SQLCreateFunctionStatement parseCreateFunction() {
     ...
     */
    override public SQLCreateProcedureStatement parseCreateProcedure() {
        /**
    ...
    private void parseProcedureStatementList(List!(SQLStatement) statementList, int max) {
        ...
        for (; ; ) {
            ...
            if (lexer.token() == Token.VARIANT) {
                ...
                if (cast(SQLBinaryOpExpr)(variant) !is null) {
                    SQLBinaryOpExpr binaryOpExpr = cast(SQLBinaryOpExpr) variant;
    ...
    public MySqlChecksumTableStatement parseChecksum() {
        ...
        for (;;) {
            SQLExprTableSource table = cast(SQLExprTableSource) this.createSQLSelectParser().parseTableSource();
            stmt.addTable(table);
     ...
     */
    override public SQLIfStatement parseIf() {
        accept(Token.IF);
     ...
     */
    override public SQLWhileStatement parseWhile() {
        accept(Token.WHILE);
     ...
     */
    override public MySqlCaseStatement parseCase() {
     ...
     */
    override public SQLStatement parseDeclare() {
        char markChar = lexer.current();
        int markBp = lexer.bp();
     ...
     */
    override public MySqlLeaveStatement parseLeave() {
        accept(Token.LEAVE);
     ...
     * @return
     */
    override public MySqlRepeatStatement parseRepeat() {
    ...
    public MySqlCursorDeclareStatement parseCursorDeclare() {
        ...
        //SQLSelectStatement selelctStmt = cast(SQLSelectStatement) parseSelect();
        SQLSelect select = this.createSQLSelectParser().select();
    ...
    public MySqlDeclareHandlerStatement parseDeclareHandler() {
        ...
        if (!(cast(SQLBlockStatement)(stmt.getSpStatement()) !is null )) {
            accept(Token.SEMI);
        }
    ...
    public MySqlDeclareConditionStatement parseDeclareCondition() {
    	...
    	*/
        ...
        stmt.setConditionName((cast(Object)(exprParser.name())).toString());
        ...
        string tokenName = lexer.stringVal();
        ConditionValue condition = new ConditionValue();
        if (equalsIgnoreCase(tokenName, "SQLSTATE")) { //for SQLSTATE (SQLSTATE '10001')
            condition.setType(ConditionValue.ConditionType.SQLSTATE);
            lexer.nextToken();
            condition.setValue((cast(Object)exprParser.name()).toString());
        } else if (lexer.token() == Token.LITERAL_INT) {
